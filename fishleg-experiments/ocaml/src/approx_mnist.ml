...
module Net = Network.MLP (struct
  let activation_fun ~layer = if layer = 2 || layer = 5 then Fn.id else AD.Maths.relu
end)

module Lik = Likelihood.Bernoulli
module F = Approx.Block_Kronecker

(* module F = Approx.Diag (Net.W) *)
...
module S =
  Setup.Make
    ...
    (struct
      ...
      let eta_fl = Setup.LR.none
      let eta_fl_aux = lrate Cmdargs.(get_float "-eta_fl_aux" |> default 3e-3)
      let bs = 10
      let bs_aux = 10
      let bs_curvature = 100
      let beta = 0.9
      let weight_decay = Some 1e-5

      let aux_weight_decay =
        None (* Some Cmdargs.(get_float "-aux_weight_decay" |> default 0.001) *)
...
let n = 784
let sizes = [| n; 300; 100; 30; 100; 300; n |]
...
let lambda () : M.L.t =
  ...
  (* let net = F.init ~scale:S.scale (w ()).net in *)
  let lik = () in
  { net; lik }
