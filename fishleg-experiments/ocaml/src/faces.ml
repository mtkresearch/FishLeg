...
module S =
  Setup.Make
    (M)
    (struct
      let eta_sgd_ref = 0.01
      let eta_fl_ref = Cmdargs.(get_float "-eta_fl" |> default 0.05)
      let eta_sgd = lrate Cmdargs.(get_float "-eta_sgd" |> default 3E-5)
      let eta_sgdm = lrate Cmdargs.(get_float "-eta_sgdm" |> default 0.03)
      let eta_adam = lrate Cmdargs.(get_float "-eta_adam" |> default 3E-5)
      let eta_fl = lrate eta_fl_ref

      let eta_fl_aux k =
        if k % update_lambda_every = 0
        then lrate Cmdargs.(get_float "-eta_fl_aux" |> default 1e-4) k
        else None
...
let _ =
  Bos.Cmd.(v "cp" % "-f" % "src/faces.ml" % S.in_dir "faces.ml" |> Bos.OS.Cmd.run)
  |> ignore

let n = 625
let sizes = [| n; 2000; 1000; 500; 30; 500; 1000; 2000; n |]

let set typ =
  let x = Owl.Mat.load_npy "_data/faces.npy" in
  let slice =
    match typ with
    | `train -> [ 0; 103_500 - 1 ]
    | `test -> [ 103_500; -1 ]
...
let w () : M.W.t =
  ...
  else { net = Net.init sizes; lik = Prms.const (AD.F 1.) }

let lambda () : M.L.t =
  let scale = S.scale in
  let net = F.init_diag ~with_bias:true ~scale sizes in
...
module One_run () = struct
  ...
  let things_to_log = 100, [ compute_mse train_set; compute_mse test_set; extract_sigma ]
